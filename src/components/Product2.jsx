  ...
  return (
    <>
      <HStack mt={{ base: 8, md: 20 }} w="100%">
        <Flex w="100%" justifyContent="space-evenly" flexDirection={{ base: "column", md: "row" }}>
          ...
          <Box bg="green" w={{ base: "100%", md: "48%" }} mt={{ base: 8, md: 0 }}>
            <Box mb={-10} display={"flex"} justifyContent={"flex-end"}>
              <Button borderRadius={"100%"} fontSize={20}>
                {videoControl ? <BiPause /> : <BiPlay />}
              </Button>
            </Box>
            <video src={video1} autoPlay loop muted style={{ width: "100%", borderRadius: 8 }}></video>
          </Box>
        </Flex>
      </HStack>
    </>
